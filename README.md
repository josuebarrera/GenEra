...
Additionally, GenEra requires one of the following two files:
  ...
  -p	A pre-generated BLAST/DIAMOND/MMseqs2 table. This file is automatically generated by genEra in the first step of the pipeline. The name of your query genes should be located in the first column of the file, and the NCBI taxonomy IDs of the target sequences should be located in the last column of the file. The user can include additional columns in between those two, but genEra will just ignore them.
...
```
The user can also incorporate two other input files, which are optional but can potentially improve the final age assignation:
