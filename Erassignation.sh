...
### Erassignation.sh v1.0 (C) Max Planck Society for the Advancement of Science
###
### Code developed by Josué Barrera-Redondo <josue.barrera@tuebingen.mpg.de>
###
...
# USAGE: Erassignation.sh [DIAMOND/BLAST results with the query sequence in the first column and the NCBI taxids in the last column] [modified "ncbi_lineages" file generated from ncbitax2lin] [taxid of query species] [percentage threshold of incomplete phylostrata] [/path/to/tmp_files] [boolean to print the deepest homolog for each gene] [oprional table with NCBI taxonomy IDs and evolutionary distances for abSENSE]
# Example: Erassignation.sh Diamond_results.txt ncbi_lineages_2021-10-26.csv 7227 30 /path/to/tmp true evolutionary_distances.tsv
...
for GENE in $(cat ${TMP_PATH}/tmp_gene_list); do
	...
	awk -v GENE="$GENE" '{ if ($1 == GENE) print $NF }' ${TMP_PATH}/tmp_${GENE}.bout | sort -u | sed '/^[[:space:]]*$/d' | awk -F "," 'FNR==NR{ a[$1]=$0;next } ($1 in a)' - ${NCBITAX} | grep "$OLDEST" > ${TMP_PATH}/tmp_phylostrata
	...
	if [ -s ${TMP_PATH}/tmp_phylostrata ]; then
		...
		for STRATUM in $(echo $QPHYLOS | sed "s/,/\n/g" | tail -n +2); do
			...
			if [ $TOTALTAXA == $MATCHINGTAXA ] && [ $COUNTER -eq 2 ]; then
				...
				echo -e "$GENE\t$STRATUM\t$PHYLORANK\t100" >> ${QTAXID}_phylostrata_assignation.tsv

				if [[ "${PRINTOLDEST}" = "true" ]]; then
					sort -nk4 ${TMP_PATH}/tmp_${GENE}.bout | tail -1 >> ${QTAXID}_deepest_homolog.tsv
				fi
			...
			# Once all the taxonomic comparisons match a certain phylostratum above the species level, check the taxonomic representativeness in the previous strata to evaluate possible HGT events (or contamination)
				...
				if [[ "${PRINTOLDEST}" = "true" ]]; then
					...
					awk -F "," -v INVERTEDCOUNTER="$INVERTEDCOUNTER" -v TAXLEVEL="$TAXLEVEL" '{ if ($INVERTEDCOUNTER != TAXLEVEL) print $1 }' ${TMP_PATH}/tmp_phylostrata | awk 'FNR==NR{ a[$1]=$0;next } ($NF in a)' - ${TMP_PATH}/tmp_${GENE}.bout | sort -nk4 | tail -1 >> ${QTAXID}_deepest_homolog.tsv
				fi
				...
				if [ $PROPORTIONSTRATA -gt $THRESHOLD ]; then
					echo -e "$GENE\t$STRATUM\t$PHYLORANK\t$PROPORTIONSTRATA" >> ${QTAXID}_phylostrata_assignation.tsv

				# Otherwise flag the gene as potential contamination or HGT event.
				else
					echo -e "$GENE\tPossible contamination or HGT\tNA\t$PROPORTIONSTRATA" >> ${QTAXID}_phylostrata_assignation.tsv

					# Create a list of possible assignable phylostrata for these ambiguous genes
					echo -e "$GENE\t${ARRAY[@]}" | sed 's/,$//' >> ${QTAXID}_ambiguous_phylostrata.tsv
				fi
			...
			# If the search for matching phylostrata doesn't reach a consesnsus at the oldest phylostratum, then assign the gene as putatively viral (this is no longer supposed to happen, but I kept it to avoid infinite loops).
				...
				echo -e "$GENE\tERROR IN PHYLOSTRATUM ASSIGNATION, PLEASE CHECK FOR ANY INCONSISTENCIES IN THE \"ncbi_lineages\" FILE\tNA\tNA" >> ${QTAXID}_phylostrata_assignation.tsv
				echo -e "ERROR IN PHYLOSTRATUM ASSIGNATION OF $GENE, PLEASE CHECK FOR ANY INCONSISTENCIES IN THE \"ncbi_lineages\" FILE"
				break
		...
		echo -e "$GENE\tAbsent from the DIAMOND/MMseqs2 results\tNA\tNA" >> ${QTAXID}_phylostrata_assignation.tsv
		if [[ "${PRINTOLDEST}" = "true" ]]; then
			echo -e "$GENE\tno_matches\tNA\tNA\tNA\tNA\tNA">> ${QTAXID}_deepest_homolog.tsv
		fi
	fi
	...
	if [[ -f ${DIVERGENCE} ]]; then
		...
		for SPECIES in $(cat ${DIVERGENCE}); do
			...
			PUTATIVE_ORTHO=$(awk -v SP_ID="$SP_ID" '{ if ($NF == SP_ID) print $(NF-1) }' ${TMP_PATH}/tmp_${GENE}.bout | sort -V | tail -1)
		...
		}' ${TMP_PATH}/tmp_${QTAXID}_${GENE}_bitscore.tsv | sed 's/ /\t/g' >> ${QTAXID}_bitscores.tsv
